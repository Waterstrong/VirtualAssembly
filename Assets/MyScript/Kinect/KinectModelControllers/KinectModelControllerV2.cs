...
public class KinectModelControllerV2 : MonoBehaviour {
    ...
    public bool MovesInSpace = true; // Waterstrong Add
    ...
    public bool VerticalMovement = false; // Waterstrong Add
    ...
    public float MoveRate = 1f; // Waterstrong Add

    // Rate at jumping // Waterstrong Add
    ...
    //private FeatureData _featureData; // feature data for motion recognition // Waterstrong Add
    //private MotionSuper _motion; // motion chain entrance // Waterstrong Add

    // Calibration Offset Variables for Character Position. // Waterstrong Add
    //bool OffsetCalibrated = false;
    float XOffset, YOffset, ZOffset;

    float currentTime; // Waterstrong Add
    bool timeRecording = false;

    //private ShowStopUI gameRunUi = null; //控制游戏运行界面的脚本对象
    ...
    // Only pulls positional, not rotational. // Waterstrong Add
    private void moveAvatar()
    {
        // Get the position of the body and store it.
        Vector3 trans = sw.playerPos[player];

        // If this is the first time we're moving the avatar, set the offset. Otherwise ignore it.
        if (!sw.offsetCalibrated)
        {
            sw.offsetCalibrated = true;

            XOffset = 0f;// trans.x * MoveRate;
            YOffset = trans.y * MoveRate; //MoveRate;//
            ZOffset = 0f;// trans.z* MoveRate;
        ...
        if (VerticalMovement)
        {
            ...
            if (yPos > 0)
            {
                yPos *= JumpRate; // When jump rate will be bigger
            }
            this.gameObject.transform.localPosition = new Vector3(xPos, yPos, zPos);
        }
        else
        {
            this.gameObject.transform.localPosition = new Vector3(xPos, 0, zPos);
        }
    }
}
